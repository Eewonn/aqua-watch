    ...
    <View style={[styles.scheduleCard, SHADOW]}>
      ...
      <View style={styles.scheduleRight}>
        ...
        <TouchableOpacity onPress={onDelete} hitSlop={10}>
          <Trash2 size={15} color="rgba(255,59,48,0.55)" />
        </TouchableOpacity>
      </View>
    </View>
  )
...
export default function FeederScreen() {
  ...
  async function handleToggle(id: string, enabled: boolean) {
    const s = await updateSchedule(id, { enabled })
    if (s) setSchedules(p => p.map(x => x.id === id ? s : x))
  }

  function handleDelete(id: string) {
    Alert.alert('Remove Schedule?', undefined, [
      { text: 'Cancel', style: 'cancel' },
      {
        text: 'Remove', style: 'destructive', onPress: async () => {
          if (await deleteSchedule(id)) setSchedules(p => p.filter(s => s.id !== id))
        },
      },
    ])
  }
